...
func newCompany(db *gorm.DB, opts ...gen.DOOption) company {
	...
	_company.companyDo.UseModel(&dbmodel.Company{})

	tableName := _company.companyDo.TableName()
	_company.ALL = field.NewAsterisk(tableName)
	_company.ID = field.NewString(tableName, "id")
	_company.TenantID = field.NewString(tableName, "tenant_id")
	_company.Name = field.NewString(tableName, "name")
	_company.CompanySize = field.NewString(tableName, "company_size")
...
func (c *company) updateTableName(table string) *company {
	...
	c.CompanySize = field.NewString(table, "company_size")
...
func (c *company) fillFieldMap() {
	...
	c.fieldMap["company_size"] = c.CompanySize
...
func (c companyDo) Create(values ...*dbmodel.Company) error {
	if len(values) == 0 {
		return nil
	}
...
func (c companyDo) CreateInBatches(values []*dbmodel.Company, batchSize int) error {
	return c.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (c companyDo) Save(values ...*dbmodel.Company) error {
	if len(values) == 0 {
		return nil
	}
...
func (c companyDo) First() (*dbmodel.Company, error) {
	if result, err := c.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*dbmodel.Company), nil
	}
}

func (c companyDo) Take() (*dbmodel.Company, error) {
...
func (c companyDo) Last() (*dbmodel.Company, error) {
...
func (c companyDo) Find() ([]*dbmodel.Company, error) {
	result, err := c.DO.Find()
	return result.([]*dbmodel.Company), err
}

func (c companyDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*dbmodel.Company, err error) {
	buf := make([]*dbmodel.Company, 0, batchSize)
...
func (c companyDo) FindInBatches(result *[]*dbmodel.Company, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (c companyDo) FirstOrInit() (*dbmodel.Company, error) {
...
func (c companyDo) FirstOrCreate() (*dbmodel.Company, error) {
...
func (c companyDo) FindByPage(offset int, limit int) (result []*dbmodel.Company, count int64, err error) {
...
func (c companyDo) Delete(models ...*dbmodel.Company) (result gen.ResultInfo, err error) {
