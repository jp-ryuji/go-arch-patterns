...
func (_c *CompanyCreate) check() error {
	...
	if _, ok := _c.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entgen: missing required field "Company.name"`)}
	}
	...
	if _, ok := _c.mutation.CompanySize(); !ok {
		return &ValidationError{Name: "company_size", err: errors.New(`entgen: missing required field "Company.company_size"`)}
	}
	if v, ok := _c.mutation.CompanySize(); ok {
	...
	if v, ok := _c.mutation.ID(); ok {
		if err := company.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entgen: validator failed for field "Company.id": %w`, err)}
		}
	}
